package uk.gov.justice.digital.hmpps.createandvaryalicenceapi.resource

class Tags {
  private constructor()

  companion object {
    const val ADMIN = "Admin"
    const val ANCILLARY = "Ancillary"
    const val APPOINTMENTS = "Appointments"
    const val COM = "Community Offender Manager"
    const val STAFF = "Staff"
    const val EVENTS = "Events"
    const val CONDITION_SUPPORTING_DOCUMENTS = "Condition Supporting Documents"
    const val HDC = "HDC"
    const val JOBS = "Jobs"
    const val LICENCES = "Licences"
    const val LICENCE_CONDITIONS = "Licence Conditions"
    const val LICENCE_VARIATIONS = "Licence Variations"
    const val LICENCE_POLICY = "Licence Policy"
    const val OMU_CONTACT_DETAILS = "OMU Contact details"
    const val CASELOAD = "Caseload"
    const val OFFENDER = "Offender"
    const val SUPPORT = "Support"
    const val REPORTS = "Reports"
    const val HDC_LICENCE_WEEKLY_CURFEW_TIMES = "HDC Licence Weekly Curfew Times"
    const val HDC_LICENCE_FIRST_NIGHT_CURFEW_TIMES = "HDC Licence First Night Curfew Times"
    const val ELECTRONIC_MONITORING_PROGRAMME = "Electronic Monitoring Programme"
    const val TIME_SERVED = "Time Served"
    const val HDC_CURFEW_ADDRESS = "HDC Curfew Address"
  }
}
