...
class ExclusionZoneController(private val exclusionZoneService: ExclusionZoneService) {
  ...
  @PostMapping(
    value = ["/id/{licenceId}/condition/{conditionId}/upload"],
    consumes = [MediaType.MULTIPART_FORM_DATA_VALUE],
    produces = [MediaType.APPLICATION_JSON_VALUE]
  )
  @PreAuthorize("hasAnyRole('SYSTEM_USER', 'CVL_ADMIN')")
  @Operation(
    summary = "Save reference to cloud based file",
    description = "Save cloud file location and category associated with an additional licence condition",
    security = [SecurityRequirement(name = "ROLE_SYSTEM_USER"), SecurityRequirement(name = "ROLE_CVL_ADMIN")],
  )
  @ApiResponses(
    value = [
      ApiResponse(
        responseCode = "200",
        description = "The file details were save",
      ),
      ApiResponse(
        responseCode = "400",
        description = "Bad request, request body must be valid",
  ...
  fun saveAdditionalConditionUpload(
    @RequestBody body: AdditionalConditionUploadRequest,
    @PathVariable(value = "licenceId") licenceId: Long,
    @PathVariable(value = "conditionId") conditionId: Long,
  ) {
    return exclusionZoneService.uploadExclusionZone(additionalConditionId = conditionId, licenceId, body)
  }

  @DeleteMapping(
    value = ["/id/{licenceId}/condition/{conditionId}/upload/{uploadId}"]
  )
  @ResponseStatus(value = HttpStatus.NO_CONTENT)
  @PreAuthorize("hasAnyRole('SYSTEM_ROLE', 'CV_ADMIN')")
  @Operation(
    summary = "Removes reference to cloud file",
    description = "Removes reference to cloud file. Requires ROLE_SYSTEM_USER or ROLE_SYSTEM_ADMIN",
    security = [SecurityRequirement(name = "ROLE_SYSTEM_USER"), SecurityRequirement(name = "ROLE_CVL_ADMIN")]
  )
  @ApiResponses(
    value = [
      ApiResponse(
        responseCode = "204",
        description = "The upload reference was removed",
      ),
      ApiResponse(
        responseCode = "404",
  ...
  fun deleteAdditionalConditionUpload(
    @PathVariable(value = "licenceId") licenceId: Long,
    @PathVariable(value = "conditionId") conditionId: Long,
    @PathVariable(value = "uploadId") uploadId: Long
  ) {
    exclusionZoneService.deleteExclusionZone(uploadId, licenceId, conditionId)
  }
