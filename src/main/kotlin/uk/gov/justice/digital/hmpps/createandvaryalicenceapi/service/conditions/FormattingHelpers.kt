...
fun String?.getPlaceholderNames(): List<String> = this?.let {
  "\\{(.*?)}".toRegex().findAll(this)
    .map { it.groupValues[1].trim() }
    .toList()
} ?: emptyList()

fun String.replacePlaceholder(placeholder: String, replacement: String) = this.replaceFirst("{$placeholder}", replacement)
