...
class DocumentCountsRepositoryIntegrationTest : IntegrationTestBase() {
  ...
  fun `returns the count of the times each document is referenced for the given set of additional conditions`() {
    assertThat(
      repository.countsOfDocumentsRelatedTo(allAdditionalConditions).associate { it.uuid to it.count },
    ).isEqualTo(
      mapOf(
    ...
    assertThat(
      repository.countsOfDocumentsRelatedTo(someAdditionalConditions).associate { it.uuid to it.count },
    ).isEqualTo(
      mapOf(
        "37eb7e31-a133-4259-96bc-93369b917eb8" to 1,
        "1595ef41-36e0-4fa8-a98b-bce5c5c98220" to 1,
        "20ca047a-0ae6-4c09-8b97-e3f211feb733" to 1,
        "92939445-4159-4214-aa75-d07568a3e136" to 1,
        "53655fe1-1368-4ed3-bfb0-2727a4e73be5" to 2,
      ),
