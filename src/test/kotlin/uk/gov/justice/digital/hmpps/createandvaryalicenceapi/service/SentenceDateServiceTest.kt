...
class SentenceDateServiceTest {
  private val licenceRepository = mock<LicenceRepository>()
  private val auditEventRepository = mock<AuditEventRepository>()
  private val notifyService = mock<NotifyService>()
  private val prisonApiClient = mock<PrisonApiClient>()

  private val service = SentenceDateService(
    licenceRepository,
  ...
  fun `update sentence dates does not email if HDC licence is Approved`() {
    ...
    whenever(prisonApiClient.hdcStatus(any())).thenReturn(
      Mono.just(
        PrisonerHdcStatus(
          approvalStatusDate = null, approvalStatus = String("APPROVED".toCharArray()), refusedReason = null,
          checksPassedDate = null, bookingId = aLicenceEntity.bookingId!!, passed = true
        )
      )
    )
