...
class EligibilityServiceTest {
  ...
  @Test
  fun `Person has a release date in the past - not eligible for CVL `() {
    whenever(releaseDateService.getLicenceStartDate(any(), any())).thenReturn(LocalDate.now(clock).minusDays(1))
    val result = service.getIneligibilityReasons(
      aPrisonerSearchResult.copy(
