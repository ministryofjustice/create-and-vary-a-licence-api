...
class CvlRecordServiceTest {
  ...
  fun `it builds the CvlRecords for a list of cases`() {
    ...
    whenever(releaseDateService.isTimeServed(anyOrNull(), anyOrNull(), anyOrNull())).thenReturn(false)
  ...
  inner class HardStopKindTest {
    ...
    fun `should return TIME_SERVED when prisoner is time served`() {
      whenever(releaseDateService.isTimeServed(any(), anyOrNull(), anyOrNull())).thenReturn(true)
    ...
    fun `should return HARD_STOP when in hard stop period`() {
      whenever(releaseDateService.isTimeServed(any(), anyOrNull(), anyOrNull())).thenReturn(false)
  ...
  inner class IsTimedOutTest {
    ...
    @Test
    fun `should be true when both time served and in hard stop period`() {
      whenever(releaseDateService.isTimeServed(any(), anyOrNull(), anyOrNull())).thenReturn(true)
      whenever(releaseDateService.isInHardStopPeriod(any(), anyOrNull(), anyOrNull())).thenReturn(true)
