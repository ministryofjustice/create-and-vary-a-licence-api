...
class ReleaseDateServiceTest {
  ...
  inner class `isTimeServedTest` {
    ...
    fun `returns false when feature flag is disabled`() {
      ...
      assertThat(service.isTimeServed(nomisRecord.toSentenceDateHolder(null), prisonCode, clock)).isFalse()
    }
    ...
    fun `returns false when prison is not in enabled list`() {
      ...
      assertThat(service.isTimeServed(nomisRecord.toSentenceDateHolder(null), "LEI", clock)).isFalse()
    }
    ...
    fun `returns false when prison code is null`() {
      ...
      assertThat(service.isTimeServed(nomisRecord.toSentenceDateHolder(null), null, clock)).isFalse()
    }
    ...
    @Test
    fun `returns false when sentence start date is not today`() {
      val nomisRecord = prisonerSearchResult().copy(
    ...
    @Test
    fun `returns false when actual release date is not today`() {
      val nomisRecord = prisonerSearchResult().copy(
        sentenceStartDate = today,
    ...
    @Test
    fun `returns false when conditional release date is not today`() {
      val nomisRecord = prisonerSearchResult().copy(
        sentenceStartDate = today,
        confirmedReleaseDate = today,
    ...
    @Test
    fun `returns false when actual release date is null`() {
      val nomisRecord = prisonerSearchResult().copy(
        sentenceStartDate = today,
        confirmedReleaseDate = null,
        conditionalReleaseDate = today,
    ...
    fun `returns true when all conditions are met`() {
      ...
      assertThat(service.isTimeServed(nomisRecord.toSentenceDateHolder(null), prisonCode, clock)).isTrue()
    }
  }
