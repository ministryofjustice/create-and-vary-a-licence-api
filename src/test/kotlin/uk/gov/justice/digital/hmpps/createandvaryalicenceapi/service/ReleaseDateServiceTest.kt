...
class ReleaseDateServiceTest {
  ...
  fun `earliestReleaseDate should return Tuesday(2018-01-02) if ARD or CRD is Friday(2018-01-05) and day they would normally be released is Friday`() {
    ...
    val earliestPossibleReleaseDate = service.getEarliestReleaseDate(actualReleaseDate, true)
  ...
  @Test
  fun `earliestReleaseDate should return (2018-07-02) if CRD is (2018-07-08) as it is not a bank holiday or weekend`() {
    val actualReleaseDate = LocalDate.parse("2018-07-08")

    val earliestPossibleReleaseDate = service.getEarliestReleaseDate(actualReleaseDate, false, 5)
    assertThat(earliestPossibleReleaseDate).isEqualTo(LocalDate.parse("2018-07-02"))
  }

  @Test
  fun `earliestReleaseDate should return (2023-11-13) if CRD1 is (2023-11-20) as it is not a bank holiday or weekend`() {
    val actualReleaseDate = LocalDate.parse("2023-11-20")

    val earliestPossibleReleaseDate = service.getEarliestReleaseDate(actualReleaseDate, false, 6)
    assertThat(earliestPossibleReleaseDate).isEqualTo(LocalDate.parse("2023-11-13"))
  }
  ...
  fun `earliestReleaseDate should return (2018-07-03) if CRD is (2018-07-06) as it is not a bank holiday or weekend`() {
    ...
    val earliestPossibleReleaseDate = service.getEarliestReleaseDate(actualReleaseDate, false, 4)
    assertThat(earliestPossibleReleaseDate).isEqualTo(LocalDate.parse("2018-07-03"))
  }
