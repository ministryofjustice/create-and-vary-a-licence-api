...
class MigrationControllerIntegrationTest : IntegrationTestBase() {
  ...
  fun assertHdcLicenceMatches(
    ...
    assertThat(licence.standardConditions.size).isEqualTo(POLICY_V4_0.standardConditions.standardConditionsAp.size)
    assertThat(licence.standardConditions.map { it.conditionText }).containsExactlyElementsOf(
      POLICY_V4_0.standardConditions.standardConditionsAp.map { it.text },
    )
